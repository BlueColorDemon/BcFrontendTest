        ...
        this.push = function (obj) {
            ...
            if (transition) {
                this.transition(from, to, false, function () {
                    J.event.fire(packageContext, 'transitionEnd', {
                        from: fromId,
                        to: id
                    });
                    callback && callback();
        ...
        var onLoginSuccess = function (data, jsonfail) {
            switch (data.retcode) {
                case 0:
                    ...
                    //     break;
                default:
                    $E.fire(packageContext, "LoginFailure", {
                        text: "登陆失败"
                    });
                    break;
            }
    ...
    J.$package("mq.presenter.buddylist", function (J) {
        ...
        var handlers = {
            ...
            onFriendsListChange: function (categories, friendsList) {
                ...
                v.memberListAreas['friend'].render({
                    type: 'category',
                    list: categories
                }, true);
            ...
            onGroupListChange: function (groupList) {
                var v = pageContext.contactView;
                v.memberListAreas['group'].render({
                    type: 'list',
                    list: groupList
                });
            },
            onDiscussListChange: function (discussList) {
                var v = pageContext.contactView;
                v.memberListAreas['discuss'].render({
                    type: 'list',
                    list: discussList
                });
            },
    ...
    J.$package("mq.model.chat", function (J) {
        ...
        this.receiveTransferMsg = function (message) {
            ...
            if (fileInfo.file_status == 51) { //对方下载超时
                content = [
					["transtimeout", fileInfo.file_name, message.lc_id]
                ];
                attach = {
                    type: 'transtimeout',
                ...
                content = [
					["transerror", fileInfo.file_name, message.lc_id]
                ];
                attach = {
                    type: 'transerror',
                    name: fileInfo.file_name,
                ...
                content = [
					["refusedbyclient", fileInfo.file_name, message.lc_id]
                ];
                attach = {
                ...
                content = [
					["transok", fileInfo.file_name, message.lc_id]
                ];
                attach = {
                    type: 'transok',
                    name: fileInfo.file_name,
        ...
        this.receiveFile = function (msg) {
            ...
            if (msg.mode === 'recv') {
				...
				*/
                ...
                var attach = filesList[_fileid];
                var content = [
					["wrfile", attach.name, attach.session_id]
                ];
                msg.content = content;
                msg.attach = {
                    type: 'wrfile',
        ...
        this.agreeReceiveFile = function (attach) {
            var content = [
				["agfile", attach.name, attach.session_id]
            ];
            attach.type = 'agfile';
            var msg = {
                from_uin: 0,
        ...
        this.refuseReceiveFile = function (attach) {
            var content = [
				["rffile", attach.name, attach.session_id]
            ];
            attach.type = 'rffile';
            var msg = {
                from_uin: 0,
        ...
        this.refuseOfflineFile = function (opt) {
            ...
            //添加拒绝消息消息
            var content = [
				['refuseofffile', '您拒绝接收“' + opt.name + '”，文件传输失败。']
            ];
            var attach = {
                type: 'refuseofffile',
                name: opt.name,
        ...
        this.nextOfflineFile = function (opt) {
            //添加拒绝消息消息 "del.png"
            var content = [
				['nextofffile', '您取消了离线文件"' + opt.name + '"的接收，我们将在您下次登录后进行提醒。']
            ];
            var attach = {
                type: 'nextofffile',
                name: opt.name,
...
define('mq.view.chat', [
    ...
    J.$package("mq.view.chat", function (J) {
        ...
        this.uploadSendFile = function (e) {
            ...
            $E.fire(packageContext, "sendFile", {
                filename: fileName,
                to_uin: currentChatUser.account,
                lcid: samp
            });
    ...
    J.$package("mq.presenter.chat", function (J) {
        ...
        var handlers = {
            ...
            onStartChat: function (opt) {
                var user, type = opt.type,
					uin = opt.uin,
					group_uin = opt.group_uin,
					discuss_uin = opt.discuss_uin;
        ...
        var getFileTypeStyle = function (fileName) {
            ...
            var fns = fileName.split("."),
				fileNameSuffix = fns[fns.length - 1].toLowerCase();
    ...
    J.$package("mq.presenter.search", function (J) {
        ...
        var handlers = {
            ...
            onSearchFriends: function (obj) {
                ...
                packageContext.view.renderResult({
                    type: 'list',
                    list: list
                });
            }
    ...
    J.$package("mq.view.search", function (J) {
        ...
        var handlers = {
            onSearchInput: function (e) {
                ...
                $E.fire(packageContext, 'searchFriends', {
                    keyword: packageContext.searchInput.value
                })
            }
        }
    ...
    J.$package("mq.view.member", function (J) {
        ...
        var commands = {
            ...
            clearSearchWord: function (panel, target, e) {
                packageContext.searchInput.value = "";
                $D.removeClass(packageContext.searchBar, "hascontent");
                $E.fire(packageContext, 'searchMembers', {
                    keyword: ""
                });
            }
        };
    ...
    J.$package("mq.presenter.setting", function (J) {
        ...
        var handlers = {
            ...
            onNotifySetting: function () {
                $E.fire(mq.view, "viewNotifySetting", {
                    from: $M('setting')
                });
            },
        ...
        var handlers = {
                ...
                for (var i = 0, r; r = result[i]; i++) {
                    switch (r.poll_type) {
                        ...
                        case "kick_message":
                            ...
                            mq.log('kick message');
                            $E.fire(packageContext, "SelfOffline", {
                                message: r.value.reason,
                                action: 'relogin'
                            });
                            break;
            ...
            onGetFirstSelfInfo: function (obj) {
                ...
                if ($D.id('mainTopAll') && !J.platform.IOS && !J.platform.android && !J.platform.winPhone) {
                    ...
                    $D.id('mainTopAll').innerHTML = tmplMainTop({
                        user: obj,
                        encode: $S.encodeHtml
                    });
